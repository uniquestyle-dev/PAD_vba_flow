...
Sub TransferTrackingNumberShopify(ByVal orderPath As String, ByVal daibikiPath As String)
    ...
    For i = 2 To lastRowDaibiki
        ...
        If targetName <> "" And trackingNum <> "" Then
            For j = 2 To lastRowOrder
                orderName = Replace(Replace(Trim(CStr(wsOrder.Cells(j, 5).Value)), " ", ""), "　", "")
                If orderName = targetName And wsOrder.Cells(j, 2).Value = "" Then
                    wsOrder.Cells(j, 2).NumberFormat = "@"
                    wsOrder.Cells(j, 2).Value = trackingNum
                    Exit For
                End If
            Next j
        End If
